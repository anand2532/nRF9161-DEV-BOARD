#!/usr/bin/env python3
"""Pass30ab — Package B' ONLY (J13+J10+J11) atomic place + full reattach.

Preferred: J13(64,75) J10(115,28) J11(115,46)
Alt1 once if preferred dirty:
         J13(64,74.5) J10(115.5,28) J11(115.5,46)
If Alt1 also dirty → full revert to pass30z keep, STOP. No Alt2.
Banned J13: (64,73), (64,78.5), (67,76)
No Package A. No U1 move. No Gerbers.
Protect Stage A, P0.15 west, RF/U3, pass30r/u, pass30z Class C copper.
"""
...
SNAP = f"{ROOT}/.mcp-backups/pass30ab-package-b"
REPORTS = f"{ROOT}/reports"
PRE = f"{SNAP}/pre-edit.kicad_pcb"
F, B = pcbnew.F_Cu, pcbnew.B_Cu
W = 0.18

# Package B' preferred / alt1 (banned: 64,73 / 64,78.5 / 67,76)
PREF = {"J13": (64.0, 75.0), "J10": (115.0, 28.0), "J11": (115.0, 46.0)}
ALT1 = {"J13": (64.0, 74.5), "J10": (115.5, 28.0), "J11": (115.5, 46.0)}
START = {"J13": (64.0, 76.0), "J10": (116.0, 28.0), "J11": (116.0, 46.0)}
BANNED_J13 = {(64.0, 73.0), (64.0, 78.5), (67.0, 76.0)}

# Same net lists as pass30y REATTACH
REATTACH = {
    "J13": [
...
def class_c_protect(board):
    """Verify pass30z Class C C22/C23/C24 stubs + GND vias still present."""
...
def protect_ok(board, west0):
    ...
    if abs(u1[0] - 36.0) > 0.05 or abs(u1[1] - 32.0) > 0.05:
        reasons.append(f"U1 moved {u1}")
    if not cc["ok"]:
...
def move_fps(board, targets):
    for ref, (x, y) in targets.items():
        # hard ban check
        if ref == "J13" and (round(x, 1), round(y, 1)) in {
            (round(a, 1), round(b, 1)) for a, b in BANNED_J13
        }:
            raise RuntimeError(f"banned J13 landing {(x, y)}")
        fp = board.FindFootprintByReference(ref)
...
def apply_package(targets, label):
    ...
    nbus = also_shift_column_bus(
        board, START["J10"][0], targets["J10"][0], 27.0, 55.0, bus_nets, tol=0.40
    )
...
def accept_attempt(info, mid, west0, before_unc):
    """Accept if clean DRC + protect OK + unconnected ≤ baseline (allow = baseline)."""
...
def main():
    ...
    summary = {
        ...
        "alt1": {k: list(v) for k, v in ALT1.items()},
        "banned_j13": [list(p) for p in sorted(BANNED_J13)],
        "no_gerbers": True,
        "packages_skipped": ["A", "C", "D", "Alt2"],
        "no_u1_move": True,
        "baseline": "pass30z KEEP unconnected=64",
        "backup": PRE,
        "live_xy_before": {},
        "landing_kept": None,
        "phase": "INIT",
    ...
    board = load()
    for ref in ["J13", "J10", "J11", "U1", "C22", "C23", "C24"]:
    ...
    for ref, exp in START.items():
        ...
        if abs(live[0] - exp[0]) > 0.05 or abs(live[1] - exp[1]) > 0.05:
            ...
            json.dump(summary, open(f"{REPORTS}/PASS30AB_SUMMARY.json", "w"), indent=2)
    ...
    before_data, before = run_drc(f"{REPORTS}/DRC_PASS30AB_BEFORE.json")
    ...
    # --- Attempt 1: preferred ---
    restore_pre()
    info = apply_package(PREF, "preferred_J13_64_75")
    mid_data, mid = run_drc(f"{REPORTS}/DRC_PASS30AB_MID_PREF.json")
    info["mid"] = mid
    ...
    if ok:
        ...
        summary["j13_landing_kept"] = "preferred (64, 75)"
    else:
        info["reject_reason"] = reason
        print("PREF rejected:", reason, "→ try ALT1 once", flush=True)
        restore_pre()
        info2 = apply_package(ALT1, "alt1_J13_64_74.5")
        mid2_data, mid2 = run_drc(f"{REPORTS}/DRC_PASS30AB_MID_ALT.json")
        info2["mid"] = mid2
        ...
        print("MID_ALT1", mid2, "clean", info2["clean"], "protect", info2["protect"]["ok"], flush=True)
        shutil.copy2(BOARD, f"{SNAP}/after-alt1.kicad_pcb")

        ok2, reason2 = accept_attempt(info2, mid2, west0, before["unconnected_items"])
        if ok2:
            kept = "alt1"
            summary["landing_kept"] = "alt1"
            summary["j13_landing_kept"] = "alt1 (64, 74.5)"
        else:
            ...
            summary["stop_reason"] = (
                f"preferred dirty/reject ({reason}); Alt1 also dirty/reject ({reason2}) "
                "— full revert to pass30z keep; STOP (no Alt2)"
            )

    after_data, after = run_drc(f"{REPORTS}/DRC_PASS30AB_AFTER.json")
    board = load()
    summary["attempts"] = []
    for a in attempts:
    ...
    lines = [
        ...
        f"**Final XY:** J13={summary['final_xy']['J13']} J10={summary['final_xy']['J10']} "
    ...
    for a in summary["attempts"]:
        ...
        lines.append(
            f"- ripped={a['ripped']} reattached={a['reattached_segs']} bus={a['bus_shifted']}"
        )
        if a.get("reject_reason"):
            lines.append(f"- reject: {a['reject_reason']}")
        if a.get("collide"):
            lines.append(f"- collide head: {json.dumps(a['collide'][:3])[:600]}")
        lines.append("")
    lines.append("## Protect checklist (after)")
    lines.append("")
    lines.append("```")
    lines.append(json.dumps(summary["protect_after"], indent=2)[:2000])
    lines.append("```")
    lines.append("")
    if summary.get("stop_reason"):
        lines.append(f"**Stop reason:** {summary['stop_reason']}")
    lines.append(f"**Backup:** `{PRE}`")
    lines.append(f"**Script:** `scripts/final_pass30ab.py`")
    lines.append(
        "**DRC:** `reports/DRC_PASS30AB_BEFORE.json`, `MID_PREF`/`MID_ALT` as needed, "
        "`DRC_PASS30AB_AFTER.json`"
    )
    open(f"{REPORTS}/PASS30AB_SUMMARY.md", "w").write("\n".join(lines) + "\n")
    print("PHASE", summary["phase"], flush=True)
    print("GATE", summary["success_gate"], flush=True)
    print("FINAL_XY", summary["final_xy"], flush=True)
    print("LANDING", summary["landing_kept"], flush=True)
